...
from agent_server.prompt_engineer import prompt_engineer_a2a_function
...
async def setup_a2a_server():
    ...
    registry = SimpleA2ARegistry()  # Register all agents
    ...
    registry.register_a2a_agent("prompt_engineer", prompt_engineer_a2a_function)
...
async def run_prompt_engineer_demo():
    """Demonstrate prompt engineer agent functionality."""
    registry = await setup_a2a_server()
    ...
    try:
        test_queries = [
            ...
            "Analysiere das Sentiment in diesem Text: Das Produkt ist fantastisch!",
        ...
        print("\n--- Prompt Engineer Agent Demo ---")

        for i, query in enumerate(test_queries, 1):
            print(f"\n{i}. Query: {query}")
            result = await registry.call_agent("prompt_engineer", query)

            if hasattr(result, "optimized_prompt"):
                opt = result.optimized_prompt
                print(f"   Detected Intent: {opt.detected_intention}")
                print(f"   Confidence: {opt.confidence_score:.2f}")
                print(f"   Recommended Agents: {opt.recommended_agents}")
                print(f"   Execution Plan: {' → '.join(result.execution_plan)}")
                print(f"   Optimized Prompt (excerpt): {opt.optimized_prompt[:150]}...")
            else:
...
async def main():
    ...
    for key, value in result.items():
        print(f"{key}: {value}")  # Demo 3: User interface agent
    await run_user_interface_demo()

    # Demo 4: Prompt engineer agent
    await run_prompt_engineer_demo()
